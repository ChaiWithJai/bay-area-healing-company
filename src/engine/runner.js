...
export async function runWorkflow({spec,config,providerName,inputDir,outputDir,runId,signal,onProgress=()=>{},prepare=prepareWorkflow,finalize=finalizeWorkflow,providerFactory=getProvider,readTelemetry}) {
  const store=new RunStore(config.stateDir),id=runId??randomUUID();
  let leases,runLease,stopSampling,poll,timer;const ctl=new AbortController();
  ...
  try {
    ...
    leases=new CoordinatorLeases(path.join(config.stateDir,'coordinator-leases.sqlite'));
    const claim=leases.tryAcquire({resource:`run:${id}`,runId:id});
    if(!claim.acquired)throw new Error(`Another coordinator is active for run ${id}; no run state changed`);
    runLease=claim.lease;
    // Refresh after acquiring ownership; the pre-lock snapshot is not authoritative.
    run=store.getRun(id);
    ...
    store.event(id,'lease.acquired',{scope:'run',resource:runLease.resource,reclaimedDeadOwner:runLease.reclaimedDeadOwner,coordinatorPid:process.pid,remoteBackendIdleProven:false});
    ...
    for(const item of plan.items){
      ...
      let feedback='',modelFeedback='';const capacityDeniedWorkers=new Set();const first=providerName??run.forcedProfile??config.routing.families[item.family]??config.defaultProvider;
      const profiles=[first,...config.routing.alternatives.filter(p=>p!==first)];let selectedProfiles=0;
      for(const profile of profiles){
        if(results[item.id])break;
        const profileWorker=config.providers?.[profile]?.workerId??'coordinator';
        if(capacityDeniedWorkers.has(profileWorker)){store.event(id,'routing.capacity_skipped',{stepId:item.id,profile,workerId:profileWorker,reason:'worker_already_denied_for_this_step'});continue;}
        if(selectedProfiles++>=2)break;
        ...
        for(let repair=0;repair<=config.routing.maxRepairs;repair++){
          ...
          const prompt=item.prompt+(modelFeedback?`\nPrevious response failed validation: ${modelFeedback}. Correct the response using only the source evidence.`:'');
          ...
          const workerId=config.providers?.[profile]?.workerId??'coordinator',waitStart=Date.now();
          store.event(id,'lease.waiting',{scope:'worker',workerId,stepId:item.id,profile});
          let workerLease;
          try{workerLease=await leases.acquire({resource:`worker:${workerId}`,workerId,runId:id,signal:ctl.signal,timeoutMs:config.limits.workerLeaseWaitMs??config.limits.runTimeoutMs});}
          catch(error){
            if(error.code!=='WORKER_QUARANTINED')throw error;
            feedback=error.message;capacityDeniedWorkers.add(workerId);
            store.event(id,'routing.worker_quarantined',{stepId:item.id,profile,workerId,reason:feedback});
            onProgress({runId:id,stepId:item.id,profile,status:'worker_quarantined',reason:feedback});break;
          }
          try {
          store.event(id,'lease.acquired',{scope:'worker',workerId,stepId:item.id,profile,waitDurationMs:Date.now()-waitStart,reclaimedDeadOwner:workerLease.reclaimedDeadOwner,coordinatorPid:process.pid,remoteBackendIdleProven:false});
          try {
            await awaitAdmission({config,workerId,signal:ctl.signal,readTelemetry,onEvent:(type,data)=>store.event(id,type,{...data,stepId:item.id,profile})});
          }catch(error){
            if(error.code!=='ADMISSION_BLOCKED')throw error;
            feedback=error.message;capacityDeniedWorkers.add(workerId);
            store.event(id,'routing.capacity_blocked',{stepId:item.id,profile,workerId,reason:feedback});
            onProgress({runId:id,stepId:item.id,profile,status:'capacity_blocked',reason:feedback});
            break;
          }
          ...
          try {
            ...
            feedback=error.message;modelFeedback=feedback;
            if(error.metrics?.workerIdleUncertain){
              const quarantine=leases.quarantineOwned(workerLease);capacityDeniedWorkers.add(workerId);
              store.event(id,'worker.quarantined',{workerId,stepId:item.id,profile,quarantine,remoteBackendIdleProven:false});
            }
            ...
            if(error.metrics?.workerIdleUncertain)break;
            if(/not installed|disabled|unavailable|HTTP|fetch failed|prohibited/i.test(feedback))break;
          }
          } finally {
            const released=leases.release(workerLease);
            store.event(id,'lease.released',{scope:'worker',workerId,stepId:item.id,profile,released});
          }
        }
      }
    ...
    if(runLease&&store.getRun(id)){const cancelled=ctl.signal.aborted&&/cancel|abort/i.test(String(ctl.signal.reason));store.updateRun(id,{status:cancelled?'cancelled':'blocked',error:error.message});store.event(id,'run.interrupted',{reason:error.message,cancelled});}
    error.runId=id;throw error;
  } finally {
    clearInterval(poll);clearTimeout(timer);signal?.removeEventListener('abort',forward);
    try {if(stopSampling)await stopSampling();} finally {
      try {if(runLease){const released=leases.release(runLease);if(store.getRun(id))store.event(id,'lease.released',{scope:'run',released});}} finally {leases?.close();store.close();}
    }
  }
}
