...
function admissionSummary(events) {
 const checks=new Map(),waits=new Map();
 const key=(event,id)=>JSON.stringify([event.runId,event.data[id]??event.id]);
 for(const e of events) {
  if(e.type==='admission.checked')checks.set(key(e,'checkId'),e);
  if(e.type==='admission.wait.started'&&!waits.has(key(e,'waitId')))waits.set(key(e,'waitId'),{...e,data:{...e.data,status:'incomplete',durationMs:null}});
 }
 for(const e of events.filter(e=>e.type==='admission.wait.finished'))waits.set(key(e,'waitId'),e);
 const checked=[...checks.values()],waited=[...waits.values()];
 const duration=list=>{const values=list.map(e=>e.data.durationMs).filter(valid);return {sum:values.length?values.reduce((a,b)=>a+b,0):null,known:values.length,total:list.length,coverage:list.length?values.length/list.length:null};};
 const aggregateAdmission=(c,w,skipped)=>({checks:c.length,admittedChecks:c.filter(e=>e.data.admit===true).length,deniedChecks:c.filter(e=>e.data.admit===false).length,unknownChecks:c.filter(e=>typeof e.data.admit!=='boolean').length,waits:w.length,incompleteWaits:w.filter(e=>e.data.status==='incomplete').length,admittedWaits:w.filter(e=>e.data.status==='admitted').length,blockedWaits:w.filter(e=>e.data.status==='blocked').length,cancelledWaits:w.filter(e=>e.data.status==='cancelled').length,invalidWaits:w.filter(e=>e.data.status==='invalid').length,skipped,waitDurationMs:duration(w),checkDurationMs:duration(c)});
 const skipped=events.filter(e=>e.type==='admission.skipped');
 const worker=e=>workerIdentity({workerId:e.data.workerId}).workerId;
 const byWorker=Object.fromEntries([...new Set([...checked,...waited,...skipped].map(worker))].map(id=>[id,aggregateAdmission(checked.filter(e=>worker(e)===id),waited.filter(e=>worker(e)===id),skipped.filter(e=>worker(e)===id).length)]));
 return {...aggregateAdmission(checked,waited,skipped.length),byWorker,scope:'Admission telemetry and polling only; separate from inference attempts/tokens. Check duration is contained within wait duration; never add these durations together.'};
}
...
export function summarizeUsage(store,{runId}={}) {
  ...
  return {schemaVersion:1,runCount:runs.length,acceptedRuns:accepted,workflowRunCount:workflowRuns.length,qualification:{runCount:runs.filter(r=>runCategory(r)==='qualification').length,...aggregate(attempts.filter(a=>a.category==='qualification'))},runtimePolicy:{localOnlySubstantiated:runs.length>0&&runs.every(localPolicyEvidence),substantiatedRunCount:runs.filter(localPolicyEvidence).length,evidence:'Saved local-only configuration: loopback inference on the coordinator or configured SSH-to-private-LAN workers validated against worker settings; not a connectivity test, network capture, or billing measurement',paidRuntimeApiConfigured:runs.length>0&&runs.every(localPolicyEvidence)?false:null},
    ...
    ...total,admission:admissionSummary(events),byWorker,byTransport:group('transport'),byModel:group('model'),byProfile:group('profile'),byWorkflow:group('workflow'),
...
export async function exportReport(store,runId,outputDir) {
  ...
  const admissionRows=[['worker_id','checks','denied_checks','waits','incomplete_waits','admitted_waits','blocked_waits','cancelled_waits','known_wait_ms','wait_duration_coverage','known_check_ms']];
  for(const [workerId,a] of Object.entries(usage.admission.byWorker))admissionRows.push([workerId,a.checks,a.deniedChecks,a.waits,a.incompleteWaits,a.admittedWaits,a.blockedWaits,a.cancelledWaits,a.waitDurationMs.sum,a.waitDurationMs.coverage,a.checkDurationMs.sum]);
  await writeFile(join(outputDir,'admission.csv'),admissionRows.map(r=>r.map(csvCell).join(',')).join('\n')+'\n',{mode:0o600});
